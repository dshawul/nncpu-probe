...
*         EXTERNAL INTERFACES
*
* Load a NNCPU file using
*
*   nncpu_init(file_path)
*
* and then probe score using one of three functions, whichever
* is convenient. From easy to hard
*   
*   a) nncpu_evaluate_fen         - accepts a fen string for evaluation
*   b) nncpu_evaluate             - suitable for use in engines
*   c) nncpu_evaluate_incremental - for ultimate performance but will need
...
* Load NNCPU file
*/
DLLExport void _CDECL nncpu_init(
  const char * evalFile             /** Path to NNCPU file */
);
...
*/
DLLExport int _CDECL nncpu_evaluate_fen(
...
/**
* Incremental NNCPU evaluation function.
* -------------------------------------------------
* First three parameters and return type are as in @nncpu_evaluate
*
* nncpu_data
*    nncpu_data[0] is pointer to NNCPUdata for ply i.e. current position
*    nncpu_data[1] is pointer to NNCPUdata for ply - 1
*    nncpu_data[2] is pointer to NNCPUdata for ply - 2
*/
DLLExport int _CDECL nncpu_evaluate_incremental(
  int player,                       /** Side to move: white=0 black=1 */
  int* pieces,                      /** Array of pieces */
  int* squares,                     /** Corresponding array of squares each piece stands on */
  NNCPUdata** nncpu_data            /** Pointer to NNCPUdata* for current and previous plies */
);
#endif
